...
function identifyUbsFirstAccess(){
 ...
 post('conecta_ubs_identificar_primeiro_acesso',{cpf:cpf,pin:pin,dispositivo:device()}).then(function(r){
  if(out){out.hidden=false;out.innerHTML='<strong>Cadastro UBS confirmado</strong><br><span class="csc-first-name">'+esc(r.nome||'Identificação indisponível')+'</span><br>'+esc(window.ConectaPerfilCadastradoLabel?window.ConectaPerfilCadastradoLabel(r.perfilCadastrado||r.perfil):'UBS')+'<br>'+esc(r.funcaoUbs||'Função não informada')+' • '+esc(r.unidadeId||'Unidade não informada');}
