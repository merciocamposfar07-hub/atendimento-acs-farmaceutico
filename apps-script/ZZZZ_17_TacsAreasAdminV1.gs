...
var TACS_TERRITORIO_V1 = Object.freeze({
  ...
  MAX_TACS:500,
  MAX_AREAS:500,
  ACCESS_PROFILES:Object.freeze([
    'ADMIN_TACS_UBS_MORADOR','ADMIN_TACS_UBS','ADMIN_UBS_MORADOR','TACS_UBS_MORADOR',
    'ADMIN_UBS','TACS_UBS','UBS_MORADOR',
    'ADMIN_TACS_MORADOR','ADMIN_TACS','ADMIN_MORADOR','TACS_MORADOR','TACS','ADMIN','UBS'
  ]),
  ...
  TACS_HEADERS:Object.freeze([
    ...
    'PIN_HASH','CRIADO_EM','ATUALIZADO_EM','OPERADOR_ATUALIZACAO','DATA_NASCIMENTO','FUNCAO_UBS'
  ]),
...
function tacsTerritorioV1ValidarSessaoToken_(p,silencioso){
  ...
  if(!tacs||!tacs.ativo||!tacsTerritorioV1PerfilTem_(tacs.perfil,'TACS')||!area||!area.ativa||area.tacsId!==tacs.tacsId){
...
function tacsTerritorioV1AdministradoresContexto_(acesso,todos){
  var out=[],vistos={};
  function add(item,perfilPadrao){
    if(!item)return;
    var nome=tacsTerritorioV1Texto_(item.nomeCompleto||item.nome||item.operadorNome||item.usuarioNome||item.displayName);
    var perfil=tacsTerritorioV1Texto_(item.perfil||item.tipo||perfilPadrao||'ADMINISTRADOR GERAL').toUpperCase();
    if(!nome||/^(ADMIN_GERAL|ADMINISTRADOR GERAL|ADMINISTRAÇÃO GERAL|AG\d+)$/i.test(nome))return;
    var key=(nome+'|'+perfil).toUpperCase();
    if(vistos[key])return;vistos[key]=true;
    out.push({nomeCompleto:nome,perfil:perfil||'ADMINISTRADOR GERAL',ativo:item.ativo!==false&&String(item.status||'').toUpperCase()!=='INATIVO'});
  }
  (todos||[]).forEach(function(item){
    if(tacsTerritorioV1PerfilTem_(item&&item.perfil,'ADMIN'))add(item,item.perfil);
  });
  ...
  /* Leitura não destrutiva de cadastros administrativos preexistentes.
     Só examina abas cujo próprio nome indica administração/usuário/operador/acesso. */
  try{
    ...
    ss.getSheets().forEach(function(sh){
      ...
      var ni=idx(['NOME_COMPLETO','NOME','ADMINISTRADOR','USUARIO_NOME','OPERADOR_NOME']);
      if(ni<0)return;
  ...
  return out.sort(function(a,b){return a.nomeCompleto.localeCompare?a.nomeCompleto.localeCompare(b.nomeCompleto,'pt-BR'):(a.nomeCompleto>b.nomeCompleto?1:-1);});
}

function tacsTerritorioV1AdministradorAtual_(acesso,todos,administradores){
  if(!acesso)return null;
  var base=acesso.base&&typeof acesso.base==='object'?acesso.base:{};
  function publicar(item,perfilPadrao){
    if(!item)return null;
    var nome=tacsTerritorioV1Texto_(item.nomeCompleto||item.nome||item.operadorNome||item.usuarioNome||item.displayName);
    if(!nome||/^(ADMIN_GERAL|ADMINISTRADOR GERAL|ADMINISTRAÇÃO GERAL|AG\d+)$/i.test(nome))return null;
    var perfil=tacsTerritorioV1Texto_(item.perfil||item.tipo||perfilPadrao||'ADMIN').toUpperCase();
    return {nomeCompleto:nome,perfil:perfil||'ADMIN',ativo:item.ativo!==false};
  }

  var direto=publicar({
    nomeCompleto:base.operadorNome||base.usuarioNome||base.nomeCompleto||base.nome||base.displayName,
...
function tacsTerritorioV1SalvarTacs_(p,acesso){
  ...
  try{
    ...
    if(id&&!existente)throw new Error('O cadastro informado não foi encontrado. Atualize o painel.');
    ...
    var perfil=tacsTerritorioV1PerfilCadastro_(body.perfil||(anterior&&anterior.perfil)||'TACS');
    var temTacs=tacsTerritorioV1PerfilTem_(perfil,'TACS');
    var temUbs=tacsTerritorioV1PerfilTem_(perfil,'UBS');
    var ubsInstitucional=perfil==='UBS';
    var nome=ubsInstitucional?'':tacsTerritorioV1Texto_(body.nomeCompleto||body.nome);
    var cns=temTacs?tacsTerritorioV1Digitos_(body.cnsProfissional||body.cns):'';
    var cpf=ubsInstitucional?'':tacsTerritorioV1Digitos_(body.cpf);
    var email=ubsInstitucional?'':tacsTerritorioV1Texto_(body.email).toLowerCase();
    var dataNascimento=ubsInstitucional?'':tacsTerritorioV1DataNascimento_(body.dataNascimento||body.nascimento);
    var telefone=ubsInstitucional?'':tacsTerritorioV1Digitos_(body.telefone||body.celular);
    var unidadeId=(temTacs||temUbs)?tacsTerritorioV1Id_(body.unidadeId||body.unidadeSaude):'';
    var microarea=temTacs?tacsTerritorioV1Texto_(body.microarea):'';
    var areaId=temTacs?tacsTerritorioV1Id_(body.areaId):'';
    var funcaoUbs=temUbs&&!ubsInstitucional?tacsTerritorioV1Texto_(body.funcaoUbs||body.funcao||body.cargo):'';
    ...
    if(!ubsInstitucional){
      ...
      if(!/^[0-9]{11}$/.test(cpf)||(typeof moradoresAdminV1CpfValido_==='function'&&!moradoresAdminV1CpfValido_(cpf)))throw new Error('Informe um CPF válido.');
      if(!/^[0-9]{10,11}$/.test(telefone))throw new Error('Informe um celular com DDD.');
    ...
    if(temUbs){
      if(!unidadeId)throw new Error('Informe a unidade de saúde do perfil UBS.');
      if(!ubsInstitucional&&!funcaoUbs)throw new Error('Informe a função do responsável na UBS.');
    }
    if(pin&&!/^[0-9]{4,8}$/.test(pin))throw new Error('O PIN individual deve conter de 4 a 8 números.');
    if(!existente&&!pin)throw new Error('Defina um PIN de acesso para o novo cadastro.');

    if(!temTacs&&anterior&&anterior.areaId){
      var areaAnterior=tacsTerritorioV1EncontrarArea_(anterior.areaId);
      if(areaAnterior&&areaAnterior.tacsId===id)throw new Error('Este cadastro ainda é o TACS responsável por uma área. Reatribua a área antes de retirar o perfil TACS.');
    }
    ...
    if(cpf){
      ...
      if(repetidoCpf&&repetidoCpf.row!==(existente&&existente.row))throw new Error('Este CPF já pertence a outro cadastro de Administrador/TACS/UBS.');
    }
    ...
    if(ativo&&(!salt||!hash))throw new Error('O acesso só pode ficar ativo depois que o PIN individual for definido.');

    var permissoes=(temTacs||temUbs)
      ?(Object.prototype.hasOwnProperty.call(body,'permissoes')
        ?tacsTerritorioV1Permissoes_(body.permissoes)
        :(anterior&&anterior.permissoes||(temTacs?TACS_TERRITORIO_V1.DEFAULT_PERMISSIONS.slice():[])))
      :[];
    ...
    var dados={
      ...
      AREA_ID:areaId,UNIDADE_ID:unidadeId,
      MICROAREA:microarea,PERFIL:perfil,
      ...
      DATA_NASCIMENTO:ubsInstitucional?'':(dataNascimento||(anterior&&anterior.dataNascimento||'')),
      FUNCAO_UBS:funcaoUbs
    };
    tacsTerritorioV1Gravar_(tabela,existente,dados);
    var depois=tacsTerritorioV1EncontrarTacs_(id);
    tacsTerritorioV1Auditar_('SALVAR_ACESSO',id,depois&&depois.areaId||'',acesso,anterior,depois);
    SpreadsheetApp.flush();
    return {ok:true,message:existente?(ubsInstitucional?'Cadastro da UBS atualizado.':'Cadastro de acesso atualizado.'):(ubsInstitucional?'UBS cadastrada.':'Administrador/TACS/UBS cadastrado.'),tacs:tacsTerritorioV1PublicarTacs_(depois)};
...
function tacsTerritorioV1PublicarTacs_(item){
  ...
  return {
    ...
    areaId:item.areaId,unidadeId:item.unidadeId,microarea:item.microarea,funcaoUbs:item.funcaoUbs||'',
...
function tacsTerritorioV1TacsDeLinha_(tabela,row){
  return {
    ...
    funcaoUbs:tacsTerritorioV1Texto_(tacsTerritorioV1Valor_(tabela,row,'FUNCAO_UBS')),
...
function tacsTerritorioV1PerfilCadastro_(valor){
  var perfil=tacsTerritorioV1Texto_(valor).toUpperCase().replace(/[+\s-]+/g,'_');
  if(perfil==='ADMIN_GERAL'||perfil==='ADMINISTRADOR'||perfil==='ADMINISTRADOR_GERAL')perfil='ADMIN';
  if(TACS_TERRITORIO_V1.ACCESS_PROFILES.indexOf(perfil)===-1)throw new Error('Selecione um dos perfis de acesso permitidos.');
  return perfil;
}
function tacsTerritorioV1PerfilTem_(perfil,vinculo){
  perfil=tacsTerritorioV1Texto_(perfil).toUpperCase();
  vinculo=tacsTerritorioV1Texto_(vinculo).toUpperCase();
  if(vinculo==='ADMIN'&&/^(ADMIN_GERAL|ADMINISTRADOR|ADMINISTRADOR_GERAL)$/.test(perfil))return true;
  return perfil.split('_').indexOf(vinculo)!==-1;
}
