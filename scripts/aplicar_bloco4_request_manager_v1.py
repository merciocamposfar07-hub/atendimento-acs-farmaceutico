...
def replace_once(path, old, new):
    ...
    if old not in s:
        raise SystemExit(f'Marcador ausente em {path}: {old[:120]}')
    if s.count(old)!=1:
        raise SystemExit(f'Marcador não único em {path}: {s.count(old)}')
...
new_refresh="""function refreshHealth(){
  ...
  jsonp('portal_manutencao_status',{areaId:context.areaId},function(r){
    mark('healthMaintenance',r&&r.ok?(r.ativa?'Ativa':'Desligada'):'Indisponível',r&&r.ok?(r.ativa?'warn':'ok'):'err');concluir();
  });
  post('admin_moradores_status',{areaId:context.areaId},function(r){
    ...
    post('admin_notificacoes_saude_rapida',{areaId:context.areaId},function(n){
      if(n&&n.ok){var c=n.contagens||{};mark('healthNotifications',String(Number(c.ativos||0))+' aptos',Number(c.reparo||0)>0?'warn':'ok')}else mark('healthNotifications','Falha','err');concluir();
...
# AGENDAS: uma leitura admin_dados confirmada por outro painel pode ser reutilizada por até 5 s, mesma identidade/área.
...
# PROFISSIONAIS: mesma leitura combinada admin_dados, mesmo namespace seguro.
...
# Teste permanente do contrato do Bloco 4.
...
# Inclui teste antes do gate final, preservando quality_gate como última barreira.
