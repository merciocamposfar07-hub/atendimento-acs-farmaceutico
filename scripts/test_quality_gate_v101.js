...
const testesEsperados = [
  'test_agenda_apps_script.js',
  'test_public_agendas_apps_script.js',
  'test_public_content_multiarea.js',
  'test_dynamic_professionals_apps_script.js',
  'test_apps_script_module_chain.js',
  'test_build_apps_script_release.js',
  'test_dom_flows.js',
  'test_admin_transport.js',
  'test_dynamic_professionals_dom.js',
  'test_public_data_transport.js',
  'test_moradores_v145.js',
  'test_portal_maintenance.js',
  'test_territorio_csv_notifications.js',
  'test_public_area_identification.js',
  'test_public_area_resolver.js',
  'test_publicacoes_territoriais.js',
  'test_territorio_dom.js',
  'test_notification_repair_button.js',
  'test_notification_repair_confirmation.js',
  'test_performance_v101.js'
];

for (const teste of testesEsperados) {
  registrar('regressao', `Suíte obrigatória encadeada: ${teste}`, contem(comandoTestes, teste));
}
...
// Integridade e segurança dos dados: otimização não pode virar alteração de regra de negócio.
registrar('dados', 'POST continua delegado ao backend anterior', contem(backend, 'return tacsPerformanceV101DoPostAnterior_(e);'));
registrar('dados', 'Cache é invalidado após mutações', contem(backend, 'tacsPerformanceV101Invalidar_();'));
...
registrar('dados', 'Ações administrativas compostas invalidam cache', contem(backend, '(salvar|criar|remover|restaurar|ativar|desativar|publicar|cancelar)'));
registrar('dados', 'Proteção contra profissional duplicado permanece', contem(profissionais, 'Profissional já cadastrado'));
registrar('dados', 'Cache público é invalidado após mutação confirmada', contem(agenda, 'portalTacsPublicInvalidateAtV1'));
...
// Desempenho e agilidade: somente leitura recebe cache; escrita permanece transacional.
registrar('desempenho', 'Cache curto para agenda pública', regex(backend, /agenda:\s*30/));
registrar('desempenho', 'Cache curto para painel público', regex(backend, /painel_publico:\s*30/));
registrar('desempenho', 'Cache para conteúdo público', regex(backend, /publico_conteudo:\s*60/));
registrar('desempenho', 'Cache curto para status administrativo', regex(backend, /admin_status:\s*15/));
