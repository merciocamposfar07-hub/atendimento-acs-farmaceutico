...
assert.match(
  performance,
  /window\.addEventListener\('pageshow',function\(\)\{\s*setTimeout\(beginPreload,120\);\s*\}\);/,
...
assert.ok(
  performance.includes("window.addEventListener('load',function(){setTimeout(beginPreload,60);setTimeout(maybePromoteBaseController,160)});"),
  'Carga inicial e restauração pageshow devem continuar caminhos separados e idempotentes'
);
