...
html body .viewer.csc-native-viewer>.csc-native-module-host:not([hidden]){display:block!important;width:100%!important;min-height:0!important;overflow:visible!important;background:#071827!important;background-image:none!important;border:0!important;box-shadow:none!important}
...
html body .viewer.csc-frame-viewer>iframe:not([hidden]){display:block!important;width:100%!important;height:100dvh!important;min-height:100dvh!important;border:0!important;background:#071827!important}
/* HOTFIX_CENTRAL_VISIBILIDADE_20260913: um módulo visível; voltar libera os toques. */
html body #viewer[hidden],html body #viewer>[hidden]{display:none!important;pointer-events:none!important}
</style>
...
function imp(n,p,v){try{if(n&&(n.style.getPropertyValue(p)!==v||n.style.getPropertyPriority(p)!=='important'))n.style.setProperty(p,v,'important')}catch(e){}}
...
function ensurePlatformFooter(){
  ...
  if(existing){if(existing.innerHTML!==FOOTER_HTML)existing.innerHTML=FOOTER_HTML;if(existing.hidden)existing.hidden=false;imp(existing,'display','grid');imp(existing,'position','static');imp(existing,'background','#071827');imp(existing,'border','0');imp(existing,'box-shadow','none')}
  var vf=document.getElementById('viewerFooter');if(vf){if(vf.innerHTML!==FOOTER_HTML)vf.innerHTML=FOOTER_HTML;var visible=viewer&&!viewer.hidden&&viewer.classList.contains('csc-native-viewer');if(vf.hidden===Boolean(visible))vf.hidden=!visible}
}
function normalizeHeader(){
  ...
  /* Visibilidade pertence ao shell. Nunca fixar display inline sobre [hidden]. */
  if(viewer&&viewer.style.getPropertyValue('display'))viewer.style.removeProperty('display');
  if(viewer&&viewer.classList.contains('csc-native-viewer')){var vbar=viewer.querySelector('.viewer-bar');imp(vbar,'position','static');imp(vbar,'top','auto');imp(vbar,'inset','auto');imp(vbar,'background','#071827');imp(vbar,'border','0');imp(vbar,'box-shadow','none')}
  if(viewer&&viewer.classList.contains('csc-frame-viewer')){var f=viewer.querySelector('.viewer-platform-footer');if(f&&!f.hidden)f.hidden=true}
}
...
function observeVisual(){if(visualObserver)visualObserver.observe(document.documentElement,{childList:true,subtree:true,attributes:true,attributeFilter:['class','hidden','style']})}
function run(){
  clearTimeout(window.__cscVisualGuardTimerV3);window.__cscVisualGuardTimerV3=0;
  if(visualObserver)visualObserver.disconnect();
  try{removeRefreshButtons();ensurePlatformFooter();normalizeHeader();installDockNavigationGuard();installBackGuard()}finally{observeVisual()}
}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',run,{once:true});else run();
setTimeout(run,120);setTimeout(run,500);setTimeout(run,1500);
try{visualObserver=new MutationObserver(function(){if(!window.__cscVisualGuardTimerV3)window.__cscVisualGuardTimerV3=setTimeout(run,40)});observeVisual()}catch(e){}
}());
</script>
'''.strip()
