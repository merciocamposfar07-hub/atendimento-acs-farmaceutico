...
function share(card,type,button,status){
  ...
  blob(type==='campanha'?drawCampaign(data):draw(data)).then(function(b){var f=new File([b],filename(data),{type:'image/png'});if(navigator.share&&(!navigator.canShare||navigator.canShare({files:[f]}))){return navigator.share({files:[f],title:data.title,text:'Portal TACS • '+data.areaName}).then(function(){if(status)status.textContent='Card compartilhado/aberto no menu do aparelho.'}).catch(function(err){if(err&&err.name==='AbortError')return;openImage(b,status)})}openImage(b,status)}).catch(function(err){if(status)status.textContent=err&&err.message||'Não foi possível criar o card.'}).finally(function(){button.disabled=false});
}
