...
var PORTAL_TACS_STATUS_ICON='/atendimento-acs-farmaceutico/conecta-saude-homologacao/v15/assets/conecta-saude-central-canonico-2026-09-09.png?v=20260920-card-csc-v1';
var PORTAL_CSC_LABEL='CONECTA SAÚDE COMUNITÁRIA';
...
function draw(data){
  return loadPortalTacsStatusIcon().then(function(portalIcon){
    ...
    if(portalIcon){ctx.drawImage(portalIcon,58,54,190,190)}
    ctx.fillStyle='#72e3a0';ctx.textBaseline='alphabetic';ctx.font='900 48px -apple-system,BlinkMacSystemFont,Arial';wrap(ctx,PORTAL_CSC_LABEL,278,116,744,54,2);
    ctx.fillStyle='#fff';ctx.font='900 34px -apple-system,BlinkMacSystemFont,Arial';ctx.fillText('RECADO DO TACS',278,207);
    var autor=txt(data.tacsName)||'Mércio José Campos dos Santos';
    ctx.fillStyle='#d8edf6';ctx.font='800 30px -apple-system,BlinkMacSystemFont,Arial';ctx.fillText(autor,278,250);
    ...
    ctx.fillStyle='#72e3a0';fitFont(ctx,PORTAL_CSC_LABEL,760,44,27,900);ctx.fillText(PORTAL_CSC_LABEL,260,1750);
...
function drawCampaign(data){
  return Promise.all([loadArt(data.theme),loadPortalTacsStatusIcon()]).then(function(assets){
    var referenceArt=assets[0],portalIcon=assets[1];
    ...
    if(portalIcon){ctx.drawImage(portalIcon,58,54,150,150)}
    ctx.fillStyle='#72e3a0';ctx.font='900 46px -apple-system,BlinkMacSystemFont,Arial';wrap(ctx,PORTAL_CSC_LABEL,235,104,790,52,2);
    ...
    ctx.fillStyle='#72e3a0';fitFont(ctx,PORTAL_CSC_LABEL,960,34,24,900);ctx.fillText(PORTAL_CSC_LABEL,60,1710);ctx.fillStyle='#fff';ctx.font='700 32px -apple-system,BlinkMacSystemFont,Arial';ctx.fillText(data.unitName,60,1760);ctx.fillText(data.areaName,60,1805);
    return c;
  });
}
...
function referenceImgHtml(theme){
  var url=artUrl(theme);return url?'<img class="camp-reference-symbol" src="'+url+'" alt="" aria-hidden="true">':'';
}
function syncAdminCampaignVisual(card){
  if(!card)return;
  var h=card.querySelector('h3'),theme=themeFromCard(card,txt(h&&h.textContent)||field(card,'titulo'));
  var html=referenceImgHtml(theme),symbol=card.querySelector('.camp-admin-symbol');
  if(symbol&&html&&symbol.dataset.referenceTheme!==theme){symbol.dataset.referenceTheme=theme;symbol.innerHTML=html}
  var period=card.querySelector('.camp-admin-period');
  if(period){
    var before=txt(period.textContent),value=before.replace(/\s*•\s*até\s+\d{2}\/\d{2}\/\d{4}/gi,'').replace(/\s*até\s+\d{2}\/\d{2}\/\d{4}/gi,'');
    if(value!==before)period.textContent=value;
  }
}

function scan(){document.querySelectorAll('#listaRecados .item[data-id]').forEach(function(c){inject(c,'recado')});document.querySelectorAll('#listaCampanhas .item[data-id]').forEach(function(c){syncAdminCampaignVisual(c);inject(c,'campanha')})}
function style(){if(document.getElementById('publicacoesWhatsappV11Style'))return;var s=document.createElement('style');s.id='publicacoesWhatsappV11Style';s.textContent='.camp-admin-symbol .camp-reference-symbol{display:block!important;width:82%!important;height:82%!important;object-fit:contain!important;max-width:82%!important;max-height:82%!important;margin:auto!important;filter:drop-shadow(0 5px 7px rgba(7,31,47,.24))!important}.publicacao-whatsapp-v11{display:grid;gap:7px;min-width:0;max-width:100%}.publicacao-campanha-status{margin:-4px 0 14px}.publicacao-recado-card{margin-top:12px}.publicacao-card-whatsapp{width:100%!important;min-height:58px!important;background:#135272!important;border:3px solid #69c7e7!important;border-radius:22px!important;color:#fff!important;font-weight:900!important;box-shadow:0 7px 18px rgba(7,58,85,.20)!important}.wa-mark{display:inline-grid;place-items:center;width:24px;height:24px;margin-right:5px;border:2px solid currentColor;border-radius:50%;font-size:12px;line-height:1}.publicacao-whatsapp-status{min-height:0;color:#536b78;font-size:.84rem;font-weight:800;line-height:1.35}.publicacao-whatsapp-status:empty{display:none}#listaCampanhas .item[hidden]+.publicacao-campanha-status{display:none!important}';document.head.appendChild(s)}
