...
function shell(name){
  return '/atendimento-acs-farmaceutico/homologacao-ui-v3/painel.html?target='+encodeURIComponent(name)+'&area='+encodeURIComponent(area())+'&from=central'+(tacsOnly()?'&acesso=tacs':'')+'&v=20260909-1';
}
function publicPortal(){return '/atendimento-acs-farmaceutico/?area='+encodeURIComponent(area())+'&from=central'}

/* Intercepta antes da navegação legada. */
document.addEventListener('click',function(e){
  var target=e.target;
  var b=target&&target.closest?target.closest('.module[data-module]'):null;
  if(!b||b.hidden||b.disabled)return;
  var name=text(b.dataset.module).toLowerCase();
  if(!name)return;
...
body.v32-admin .login-tabs{display:none!important}\
body.v32-admin #tacsLogin{display:none!important}\
body.v32-tacs .login-tabs{display:none!important}\
body.v32-tacs #adminLogin{display:none!important}\
#loginStatus.v32-passive{display:none!important}\
';(document.head||document.documentElement).appendChild(s);
}
function modeUi(){
  if(!document.body)return;
  document.body.classList.toggle('v32-tacs',tacsOnly());
  document.body.classList.toggle('v32-admin',!tacsOnly());
  ...
  if(status&&hasSession()&&!/erro|err/.test(status.className||''))status.classList.add('v32-passive');
}
function expiredMessage(){
  if(String(q().get('sessao')||'').toLowerCase()!=='expirada')return;
  var n=document.getElementById('loginStatus');
  if(n){n.classList.remove('v32-passive');n.className='status warn';n.textContent='Sessão encerrada após 1 hora sem interação. Digite seu PIN novamente.'}
}
...
var sources={
  ...
  territorio:'/atendimento-acs-farmaceutico/painel-oficial-tacs-areas.html',
  municipios:'/atendimento-acs-farmaceutico/painel-oficial-organizacoes-municipios.html'
};
var warmed={};
function warmOne(name){
  if(warmed[name]||!sources[name])return;warmed[name]=1;
  var u=sources[name]+'?area='+encodeURIComponent(area())+'&from=central'+(tacsOnly()?'&acesso=tacs':'')+'&v=20260909-warm';
  try{fetch(u,{credentials:'same-origin',cache:'force-cache'}).catch(function(){})}catch(e){}
}
function warmAll(){
  if(!hasSession())return;
  var names=Object.keys(sources),i=0;
  function next(){if(i>=names.length)return;warmOne(names[i++]);setTimeout(next,90)}
  if('requestIdleCallback'in window)requestIdleCallback(next,{timeout:700});else setTimeout(next,500);
}
function watch(){
  var obs=new MutationObserver(function(){modeUi();expiredMessage();if(hasSession()){sessionTouch();warmAll()}});
  obs.observe(document.documentElement,{subtree:true,childList:true,attributes:true,attributeFilter:['hidden','class']});
}
function boot(){style();modeUi();expiredMessage();watch();if(hasSession()){sessionTouch();warmAll()}}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
window.addEventListener('pageshow',function(){modeUi();if(hasSession()){sessionTouch();warmAll()}});
}());
