...
var UBS_TOKEN='portalConectaUbsTokenV1';
...
function openProfilePage(){
  ...
  var ctx=centralContextFromCache()||{},areas=Array.isArray(ctx.areas)?ctx.areas:[],tacs=Array.isArray(ctx.tacs)?ctx.tacs.filter(function(item){return !!text(item&&(item.nomeCompleto||item.nome));}):[],admins=Array.isArray(ctx.administradores)?ctx.administradores:[];
...
function buildCentralWelcome(){
  ...
  var existingDock=document.getElementById('cscInstitutionalDock');
  if(existingDock){
    var existingHandlers={home:showCentralHome,profile:openProfilePage};
    Array.prototype.forEach.call(existingDock.querySelectorAll('.csc-navitem'),function(b){
      var key=b.dataset.cscDock||'',fn=existingHandlers[key];
      if(b.dataset.cscDockWired==='1'||!fn)return;
      b.dataset.cscDockWired='1';
      b.addEventListener('click',function(){vibrate();existingDock.querySelectorAll('.csc-navitem').forEach(function(x){x.classList.toggle('active',x===b)});fn()});
    });
    return;
  }
  ...
  var items=[
    ['⌂','Início',showCentralHome,'home'],
    ['●','Perfil',openProfilePage,'profile']
  ];
  items.forEach(function(item,i){
    var b=document.createElement('button');b.type='button';b.className='csc-navitem'+(i===0?' active':'');b.dataset.cscDock=item[3];
