...
function buildCentralWelcome(){
  ...
  if(existingDock){
    var existingHandlers=[showCentralHome,openRecordsPage,openPendingPage,openProfilePage];
    Array.prototype.forEach.call(existingDock.querySelectorAll('.csc-navitem'),function(b,i){
      if(b.dataset.cscDockWired==='1'||!existingHandlers[i])return;
      b.dataset.cscDockWired='1';
      b.addEventListener('click',function(){vibrate();existingDock.querySelectorAll('.csc-navitem').forEach(function(x){x.classList.toggle('active',x===b)});existingHandlers[i]()});
    });
    return;
  }
