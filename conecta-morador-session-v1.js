...
async function activateNotifications(){
 ...
 if(testMode()){
  ...
  try{var tr=await post('conecta_morador_notificacao_confirmar',{token:token,dispositivo:device(),permission:'SIM',optedIn:'SIM',subscriptionId:'teste'});resident.notificacoesAtivas=true;gateStatus(tr.message||'Etapa validada no modo teste.','ok');setTimeout(function(){hideGate();removeDuplicateFamilySelector()},450)}catch(te){gateStatus(te.message,'err')}finally{if(btn)btn.disabled=false}
  return;
 }
 ...
 try{
  ...
  setTimeout(function(){hideGate();removeDuplicateFamilySelector()},650);
