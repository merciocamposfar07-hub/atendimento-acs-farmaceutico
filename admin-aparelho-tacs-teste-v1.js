(function(){
  ...
  function paginaAtiva(){try{return typeof window!=='undefined'&&typeof document!=='undefined'&&document&&document.documentElement}catch(e){return false}}
  function areaAtual(){if(!paginaAtiva())return'JAPARANDUBA';var s=document.getElementById('areaEnvio'),a=txt(s&&s.value)||new URLSearchParams(location.search||'').get('area')||'JAPARANDUBA';return String(a).toUpperCase().replace(/[^A-Z0-9_-]/g,'')||'JAPARANDUBA'}
  ...
  function sessao(){var s={dispositivo:device(),areaId:areaAtual()},t='',a='';try{t=sessionStorage.getItem(TERRITORY_TOKEN_KEY)||'';a=sessionStorage.getItem(TOKEN_KEY)||''}catch(e){}if(t)s.territorioToken=t;else if(a)s.token=a;var c=chaveTecnica(),sub=subscriptionId();if(c)s.chaveTacsTeste=c;if(sub)s.subscriptionId=sub;return s}
  ...
  function sanearHistorico(){
    if(!paginaAtiva()||operando)return;
    ...
    enviar('admin_notificacoes_aparelho_tacs_sanear_historico','').then(function(r){
      if(!paginaAtiva())return;if(!r||r.ok!==true)throw new Error(txt(r&&r.message)||'Não foi possível sanear o histórico deste aparelho.');
      operando=false;ultimoEstado=ultimoEstado||{};render(ultimoEstado);if(st&&paginaAtiva())st.textContent=txt(r.message)||'Saneamento histórico concluído.';
    }).catch(function(e){operando=false;if(paginaAtiva())render(ultimoEstado,e.message||'Não foi possível sanear o histórico deste aparelho.')});
  }
  function alternar(){if(!paginaAtiva()||operando)return;if(!ultimoEstado){consultar();return}var ativo=ultimoEstado.aparelhoTacsTeste===true&&ultimoEstado.autorizadoNesteAparelho===true,modo=ativo?'DESATIVAR':'ATIVAR',pergunta=ativo?'Voltar este aparelho ao modo morador?':'Ativar o modo TACS / teste neste aparelho? A autorização ficará vinculada a este navegador e a esta área.';if(!window.confirm(pergunta))return;operando=true;render(ultimoEstado);var b=box(),st=b&&b.querySelector('.apt-status');if(st)st.textContent=ativo?'Desativando o modo TACS / teste…':'Ativando e autorizando este aparelho…';executar(modo).then(function(r){if(!paginaAtiva())return;if(!r||r.ok!==true)throw new Error(txt(r&&r.message)||'Não foi possível atualizar este aparelho.');if(modo==='ATIVAR'){if(!r.chaveTecnica)throw new Error('O servidor não devolveu a autorização técnica deste aparelho.');salvarChave(r.chaveTecnica);r.autorizadoNesteAparelho=true}else salvarChave('');ultimoEstado=r;operando=false;render(r)}).catch(function(e){operando=false;if(paginaAtiva())render(ultimoEstado,e.message)})}
  function iniciarOneSignalOpcional(){
    if(!paginaAtiva()||oneSignalTentado)return;oneSignalTentado=true;
    window.OneSignalDeferred=window.OneSignalDeferred||[];
    window.OneSignalDeferred.push(async function(OneSignal){
      if(!paginaAtiva())return;oneSignalRef=OneSignal;
      try{
      ...
      if(!paginaAtiva())return;setTimeout(consultar,120);
      try{var push=OneSignal.User&&OneSignal.User.PushSubscription;if(push&&typeof push.addEventListener==='function')push.addEventListener('change',function(){if(paginaAtiva())setTimeout(consultar,120)})}catch(e){}
    });
  ...
  function instalar(){if(!paginaAtiva())return;box();consultar();iniciarOneSignalOpcional();var a=document.getElementById('areaEnvio');if(a)a.addEventListener('change',function(){ultimoEstado=null;if(paginaAtiva())setTimeout(consultar,150)});var sec=document.getElementById('saudeNotificacoes');if(sec&&typeof MutationObserver!=='undefined')new MutationObserver(function(){if(paginaAtiva()&&!sec.classList.contains('oculto'))setTimeout(consultar,100)}).observe(sec,{attributes:true,attributeFilter:['class']})}
