...
function loadPerformanceLayer(){if(window.PortalTacsCentralPerformanceV1||document.getElementById('portalTacsCentralPerformanceLoaderV1'))return;var script=document.createElement('script');script.id='portalTacsCentralPerformanceLoaderV1';script.src='/atendimento-acs-farmaceutico/central-admin-performance-v1.js?v=20260823-admin-performance-v1';script.async=false;document.head.appendChild(script)}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',function(){install();loadPerformanceLayer()},{once:true});else{install();loadPerformanceLayer()}window.addEventListener('pageshow',function(){install();loadPerformanceLayer()});
}());