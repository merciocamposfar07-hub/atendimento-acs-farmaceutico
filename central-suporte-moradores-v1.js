...
function openSupport(button){if(!hasAnySession())return;var viewer=document.getElementById('viewer'),frame=document.getElementById('viewerFrame'),title=document.getElementById('viewerTitle');if(!viewer||!frame)return;var url='/atendimento-acs-farmaceutico/painel-suporte-moradores.html?area='+encodeURIComponent(areaId())+(hasTerritorySession()?'&acesso=tacs':'')+'&v=20260822-suporte-moradores-v1';if(title)title.textContent='Suporte aos moradores';frame.src=url;viewer.hidden=false;document.body.classList.add('viewer-open');try{button.blur()}catch(e){}}
