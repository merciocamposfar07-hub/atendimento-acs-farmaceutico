(function(){
  ...
  function residentPinMarker(){try{return !tacsTeste()&&localStorage.getItem(PIN_MARKER_KEY)==='1'}catch(e){return false}}
  function residentServerCpfSaved(){try{var d=digits(localStorage.getItem(SERVER_CPF_HINT_KEY)||'');return /^\d{11}$/.test(d)?d:''}catch(e){return''}}
  function saveResidentServerCpf(cpf){var d=digits(cpf);if(tacsTeste()||!/^\d{11}$/.test(d))return'';residentServerCpfHint=d;try{localStorage.setItem(SERVER_CPF_HINT_KEY,d);localStorage.setItem(PIN_MARKER_KEY,'1');localStorage.setItem(LAST_ROLE_KEY,'MORADOR')}catch(e){}return d}
  function residentInstallQuick(){try{if(tacsTeste())return'';var parts=('; '+document.cookie).split('; '+INSTALL_COOKIE+'=');if(parts.length!==2)return'';var q=decodeURIComponent(parts.pop().split(';').shift()||'');return /^cmq1\./.test(q)?q:''}catch(e){return''}}
  function saveResidentInstallCookie(q){q=text(q);if(tacsTeste()||!/^cmq1\./.test(q))return;try{document.cookie=INSTALL_COOKIE+'='+encodeURIComponent(q)+'; Max-Age=31536000; Path=/atendimento-acs-farmaceutico/; Secure; SameSite=Lax'}catch(e){}}
  function syncResidentInstallCookie(){var p=residentProfile(),q=text(p&&p.quickKey);if(/^cmq1\./.test(q))saveResidentInstallCookie(q);return residentInstallQuick()}
  function residentLikelyHasPin(){try{return Boolean(residentProfile()||residentVaultExists()||residentPinMarker()||residentServerCpfSaved()||residentInstallQuick()||(!tacsTeste()&&text(localStorage.getItem(LAST_ROLE_KEY)||'').toUpperCase()==='MORADOR'))}catch(e){return Boolean(residentProfile()||residentVaultExists()||residentPinMarker()||residentServerCpfSaved()||residentInstallQuick())}}
  function recognizeResidentDevice(){
    if(tacsTeste()||residentSessionToken())return Promise.resolve(false);
    if(residentLikelyHasPin()){residentDeviceRecognized=true;renderResidentPinLogin();return Promise.resolve(true)}
    var d=deviceId(false);if(!d)return Promise.resolve(false);
    return jsonp({action:'conecta_morador_reconhecer_aparelho',dispositivo:d}).then(function(r){
      if(r&&r.ok===true&&r.temPin===true){
        residentDeviceRecognized=true;
        try{localStorage.setItem(PIN_MARKER_KEY,'1');localStorage.setItem(LAST_ROLE_KEY,'MORADOR')}catch(e){}
        renderResidentPinLogin();
        return true;
      }
      return false;
  ...
  function saveResidentAccess(r){
    try{
      ...
      else if(/^cmq1\./.test(quick)){profileKey=PROFILE_KEY;saveResidentInstallCookie(quick)}
      ...
      if(/^cmq1\./.test(quick)||/^cms1\./.test(tok)){localStorage.setItem(PIN_MARKER_KEY,'1');residentDeviceRecognized=true}
  ...
  function renderMissingCpfReview(){var teste=tacsTeste(),titulo=teste?'Modo teste: confira os dados':'Confira seus dados antes de salvar',ajuda=teste?'Esta confirmação é apenas uma simulação. Nenhuma alteração será gravada na planilha real.':'Pare um momento e verifique se os 11 números do CPF e a data de nascimento estão corretos.',botao=teste?'Confirmar teste e continuar':'Confirmar e continuar';setPinBox('<strong class="tacs-family-title">'+titulo+'</strong><p class="tacs-family-help">'+ajuda+'</p><div class="tacs-family-help"><strong>CPF:</strong> '+escapeHtml(formatDoc(residentEnrollmentCpf))+'<br><strong>Data de nascimento:</strong> '+escapeHtml(residentBirthConfirm)+'</div><div class="tacs-pin-actions"><button type="button" class="tacs-pin-action" data-resident-cpf-review-confirm="1">'+botao+'</button><button type="button" class="tacs-family-action" data-resident-cpf-review-correct="1">Corrigir</button></div>','tacs-pin-box')}
  ...
  function beginResidentPinEnrollment(cpf){
    ...
    return residentPost('conecta_morador_identificar',{cpf:d,dispositivo:deviceId(true)}).then(function(r){
      if(r&&r.jaPossuiPin===true){
        saveResidentServerCpf(d);
        renderResidentPinLogin(r.message||'Seu PIN já está criado. Digite os quatro números para acessar sua família.');
        return r;
      }
  ...
  function renderResidentPinLogin(message){
    var p=residentProfile(),disponivel=Boolean(p||residentVaultExists()||residentPinMarker()||residentInstallQuick()||residentDeviceRecognized||/^\d{11}$/.test(residentServerCpfHint));
...
function loginResidentPinFromPortal(){
 var p=residentProfile(),installQuick=residentInstallQuick(),pin=digits(document.getElementById('portalResidentPinLogin')&&document.getElementById('portalResidentPinLogin').value),btn=document.querySelector('[data-resident-pin-login]'),hintCpf=residentServerCpfHint||residentServerCpfSaved();
 if(!p&&!residentVaultExists()&&!residentPinMarker()&&!installQuick&&!residentDeviceRecognized&&!/^\d{11}$/.test(hintCpf)){hidePin();return}
 if(!/^\d{4}$/.test(pin)){renderResidentPinLogin('Digite os 4 números do seu PIN.');return}
 if(btn)btn.disabled=true;
 setPinBox('<strong class="tacs-family-title">Aguarde, o PIN está sendo reconhecido…</strong><p class="tacs-family-help">Validando seu acesso e preparando sua família.</p>','tacs-pin-box');
 ...
 local.then(function(saved){
  ...
  function install(){ensureStyle();residentServerCpfHint=residentServerCpfHint||residentServerCpfSaved();var input=document.getElementById('cpf'),status=document.getElementById('cpfStatus');if(!input||!status){setTimeout(install,120);return}var label=input.closest('label');if(label&&label.firstChild){label.firstChild.textContent='CPF, Cartão SUS (CNS) ou cadastro da família ';input.placeholder='CPF, Cartão SUS ou família (ex.: 053)'}box();docBox();pinBox();observeStatus();syncResidentInstallCookie();if(!residentSessionToken()&&residentLikelyHasPin())renderResidentPinLogin();resgatarModoTacsTeste().then(function(ok){if(ok)return;if(residentSessionToken())return;return recognizeResidentDevice().then(function(recognized){if(!recognized)renderStart()})})}
