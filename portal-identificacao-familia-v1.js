(function(){
  ...
  function loginResidentPinFromPortal(){var p=residentProfile(),pin=digits(document.getElementById('portalResidentPinLogin')&&document.getElementById('portalResidentPinLogin').value),btn=document.querySelector('[data-resident-pin-login]');if(!p){hidePin();return}if(!/^\d{4}$/.test(pin)){renderResidentPinLogin();return}if(btn)btn.disabled=true;residentPost('conecta_morador_login_pin',{quickKey:p.quickKey,pin:pin,dispositivo:deviceId(true)}).then(function(r){saveResidentAccess(r);setPinBox('<strong class="tacs-family-title">✓ PIN confirmado</strong><p class="tacs-family-help">Abrindo sua família…</p>','tacs-pin-box');var area=encodeURIComponent(r.areaId||p.areaId||areaId());setTimeout(function(){location.assign('/atendimento-acs-farmaceutico/?area='+area+'&conecta=1')},120)}).catch(function(e){setPinBox('<strong class="tacs-family-title">PIN não confirmado</strong><p class="tacs-family-help">'+escapeHtml(e.message)+'</p><input id="portalResidentPinLogin" type="password" inputmode="numeric" maxlength="4" autocomplete="current-password" placeholder="PIN de 4 números"><div class="tacs-pin-actions"><button type="button" class="tacs-pin-action" data-resident-pin-login="1">Tentar novamente</button></div>','tacs-family-warn')}).finally(function(){var b=document.querySelector('[data-resident-pin-login]');if(b)b.disabled=false})}
