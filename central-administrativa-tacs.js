...
function aquecerValidacaoPin(){
  ...
  try{last=Number(localStorage.getItem(SHARED_WARM_KEY)||0)}catch(e){}
  if(last&&now-last<45000)return;
  pinWarmupInFlight=true;
  try{
    fetch(API+'?action=publico_areas_ativas&_='+now,{method:'GET',mode:'no-cors',cache:'no-store',credentials:'omit'})
      .catch(function(){})
      .finally(function(){pinWarmupInFlight=false;marcarConexaoRecente()});
  }catch(e){pinWarmupInFlight=false}
}
...
['adminPin','tacsPin'].forEach(function(id){var input=el(id);if(!input)return;input.addEventListener('focus',function(){aquecerValidacaoPin();prefetchStaticPanels()},{once:true});input.addEventListener('input',function(){aquecerValidacaoPin();prefetchStaticPanels()},{once:true})});
