...
function ensureTask16AgendaAssets(callback){
  ...
  task16LoadScript('cscModuleCoreTask16','/atendimento-acs-farmaceutico/conecta-module-core-v1.js?v=20260916-cache-persistente-v1',function(){return Boolean(window.ConectaModuleCoreV1)},function(ok){
    if(!ok){finish(false);return}
    task16LoadScript('cscAgendaTransportTask16','/atendimento-acs-farmaceutico/conecta-agendas-transport-v1.js?v=20260920-agendas-releitura-v1',function(){return Boolean(window.ConectaAgendasTransportV1)},function(ok2){
      if(!ok2){finish(false);return}
      task16LoadScript('cscAgendaNativeTask16','/atendimento-acs-farmaceutico/conecta-agendas-native-v1.js?v=20260920-agendas-releitura-v1&load=20260920-agendas-releitura-v1',function(){return Boolean(window.ConectaAgendasNativeV1)},function(ok3){
...
function ensureTask17MoradoresAssets(callback){
  ...
  task16LoadScript('cscModuleCoreTask17','/atendimento-acs-farmaceutico/conecta-module-core-v1.js?v=20260916-cache-persistente-v1',function(){return Boolean(window.ConectaModuleCoreV1)},function(ok){
    if(!ok){finish(false);return}
    task16LoadScript('cscMoradoresNativeTask17','/atendimento-acs-farmaceutico/conecta-moradores-native-v1.js?v=20260918-moradores-app-like-instant-v4',function(){return Boolean(window.ConectaMoradoresNativeV1)},finish);
  });
}
...
function showPendingModuleShell(name,title,routeId){
  ...
  var base=el('viewerFrame');if(base)base.hidden=true;
  var viewer=el('viewer'),host=ensurePendingModuleHost();if(!viewer||!host)return false;
  ensurePendingPreviewStyle();
  shellActiveModule=name;shellActiveRoute=routeId;shellActiveNative='';
  el('viewerTitle').textContent=title||'Painel';
  viewer.classList.add('csc-shell-viewer','csc-native-viewer');viewer.classList.remove('csc-frame-viewer');viewer.hidden=false;
...
function prefetchStaticPanels(){
  ...
  var queue=[
    ...
    '/atendimento-acs-farmaceutico/conecta-agendas-native-v1.js?v=20260920-agendas-releitura-v1',
