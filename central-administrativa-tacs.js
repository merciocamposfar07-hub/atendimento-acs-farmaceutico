...
function moduleUrl(name,options){
  var area=encodeURIComponent(selectedAreaId),tacsOnly=mode==='tacs'||TACS_ONLY,access=tacsOnly?'&acesso=tacs':'',revision='20260913-apresentacao-paineis-v2',localFirstRevision='20260913-local-first-paineis-v1',territoryRevision='20260913-territorio-contexto-local-v4',loadingRevision='20260913-loading-standard-v1',from='&from=central&load='+loadingRevision,opts=moduleRouteOptions(options),extra='';
  ...
  if(name==='suporte')return '/atendimento-acs-farmaceutico/painel-suporte-moradores-v2.html?area='+area+access+extra+from+'&v='+localFirstRevision;
  if(name==='recados')return '/atendimento-acs-farmaceutico/painel-oficial-recados-campanhas.html?area='+area+access+extra+from+'&v='+localFirstRevision;
  ...
  if(name==='municipios')return '/atendimento-acs-farmaceutico/painel-oficial-organizacoes-municipios.html?from=central&v='+localFirstRevision;
...
function ensureTask16AgendaAssets(callback){
  ...
  task16LoadScript('cscModuleCoreTask16','/atendimento-acs-farmaceutico/conecta-module-core-v1.js?v=20260913-agendas-ios-mount-v1',function(){return Boolean(window.ConectaModuleCoreV1)},function(ok){
    ...
    task16LoadScript('cscAgendaTransportTask16','/atendimento-acs-farmaceutico/conecta-agendas-transport-v1.js?v=20260913-agendas-ios-mount-v1',function(){return Boolean(window.ConectaAgendasTransportV1)},function(ok2){
      ...
      task16LoadScript('cscAgendaNativeTask16','/atendimento-acs-farmaceutico/conecta-agendas-native-v1.js?v=20260913-agendas-ios-mount-v1&load=20260913-loading-standard-v1',function(){return Boolean(window.ConectaAgendasNativeV1)},function(ok3){
...
function localFirstFrameUrl(url){
  try{
    var u=new URL(url,location.origin);
    u.searchParams.set('localfirst','1');
    return u.pathname+u.search;
  }catch(e){
    return url+(url.indexOf('?')===-1?'?':'&')+'localfirst=1';
  }
}
...
function openModule(name,title,options){
  ...
  if(!remoteReady){
    ...
    /* Painéis em frame também abrem a tela real no primeiro toque.
       O parâmetro localfirst libera somente leitura/caches; gravações continuam
       dependentes da sessão remota confirmada. */
    if(name==='territorio'||name==='suporte'||name==='recados'||name==='municipios'){
      if(name==='territorio')priorizarSincronizacaoTerritorioPendente();
      var localFrame=ensureShellFrame(name,localFirstFrameUrl(url),title||'Painel',routeId);
  ...
  if(frame.dataset.shellLocalFirst==='1'){
    /* O mesmo painel permanece visível; a própria página observa a chegada
       da sessão remota e sincroniza sem recarregar ou apagar a tela. */
